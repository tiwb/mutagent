...
class TestCollectNamespaces:
    ...
    def test_empty_registry_returns_only_decl(self):
        ...
        assert _external_names(sandbox) == set()
    ...
    def test_exec_and_share_see_same_names(self):
        ...
        sandbox = SandboxEnv()
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_alpha",
                                   functions={"f": lambda: 1}))
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_beta",
                                   functions={"g": lambda: 2}))
        # 两端看到的 name 集合（排除 exec 路径的 help 键）
        exec_names = {k for k in _build_namespace_dict(sandbox) if k != "help"}
        share_names = set(_all_namespaces(sandbox).keys())
...
# ---------------------------------------------------------------------------
# SandboxEnv.iter_namespaces / get_namespace
# ---------------------------------------------------------------------------
...
class TestIterNamespaces:
    ...
    def test_iter_sorted_by_name(self):
        sandbox = SandboxEnv()
        sandbox_env_add_namespace(sandbox, _make_ns("zzz_last",
                                   functions={"f": lambda: 1}))
        sandbox_env_add_namespace(sandbox, _make_ns("aaa_first",
                                   functions={"f": lambda: 1}))
...
class TestGetNamespace:
    ...
    def test_returns_none_for_missing_name(self):
        sandbox = SandboxEnv()
        assert sandbox_env_get_namespace(sandbox, "__definitely_not_registered__") is None

    def test_returns_none_for_help_key(self):
        """'help' 是注入沙箱的特殊键，不应被 get_namespace 暴露。"""
        sandbox = SandboxEnv()
        assert sandbox_env_get_namespace(sandbox, "help") is None

    def test_returns_namespace_for_single_provider(self):
        sandbox = SandboxEnv()
        ns = _make_ns("uniq_get_single", functions={"f": lambda: 1})
        sandbox_env_add_namespace(sandbox, ns)
        got = sandbox_env_get_namespace(sandbox, "uniq_get_single")
        assert isinstance(got, Namespace)
        assert got is ns

    def test_returns_merged_view_for_multi_provider(self):
        sandbox = SandboxEnv()
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_get_multi",
                                   functions={"a": lambda: 1}))
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_get_multi",
                                   functions={"b": lambda: 2}))
        got = sandbox_env_get_namespace(sandbox, "uniq_get_multi")
...
class TestSandboxBoundHelp:
    ...
    def test_help_no_arg_lists_external(self):
        sandbox = SandboxEnv()
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_help_list",
                                   desc="demo",
                                   functions={"do": lambda: 1}))
        help_fn = sandbox.exec_code("_r = help()")["state"]["_r"] if False else None  # noqa
        # 用 _build_namespace_dict 直接拿 help
        ns_dict = _build_namespace_dict(sandbox)
    ...
    def test_help_string_lookup(self):
        ...
        sandbox = SandboxEnv()
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_help_string",
                                   functions={"do": do}))
        help_fn = _build_namespace_dict(sandbox)["help"]
    ...
    def test_help_multi_provider_shows_merged_view(self):
        sandbox = SandboxEnv()
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_help_merged",
                                   functions={"a": lambda: 1}))
        sandbox_env_add_namespace(sandbox, _make_ns("uniq_help_merged",
                                   functions={"b": lambda: 2}))
        help_fn = _build_namespace_dict(sandbox)["help"]
