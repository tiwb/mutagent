...
1. ``_wrap_async`` 同步路径（pysandbox tool entry）—— worker 线程同步
   ...
   目标 loop 同线程同步调用抛错。
2. ``share.py:_handle_call`` 异步路径（pysandbox/namespaces.call RPC）——
   handler 已在主 loop async 上下文，应通过 ``_async_original`` 直接
   await 原 coroutine，绕过 sync wrapper，**不依赖 bind_main_loop()**。
3. ``SandboxApp.bind_main_loop()`` helper —— 写入 ``_async_loop`` /
...
class TestHandleCallAsyncPath:
    ...
    async def test_handle_call_awaits_async_original_without_bind(self):
        """share.py 路径已在主 loop async 上下文，应直接 await 原 coroutine，
        即使 SandboxApp 从未 bind_main_loop() 也必须工作（不能踩 sync wrapper）。

        这是导致用户看到 ``MCP error -32603: SandboxApp._async_loop not set``
        的根因路径，必须有回归覆盖。
        """
        from mutio.mcp.protocol import JsonRpcDispatcher
        from mutagent.sandbox.share import register_pysandbox_methods

        app = SandboxApp()
        # 故意不 bind_main_loop()：验证 share.py 路径不依赖它
        ...
        register_pysandbox_methods(dispatcher, app)
...
class TestBindMainLoop:
    ...
    async def test_bind_main_loop_writes_loop_and_thread_id(self):
        ...
        assert getattr(app, "_async_loop") is asyncio.get_running_loop()
        assert getattr(app, "_async_loop_thread_id") == threading.get_ident()

    @pytest.mark.asyncio
    async def test_bind_main_loop_is_idempotent(self):
        app = SandboxApp()
        app.bind_main_loop()
        loop1 = app._async_loop  # type: ignore[attr-defined]
        app.bind_main_loop()
        loop2 = app._async_loop  # type: ignore[attr-defined]
